...
instance Monad m => ProfunctorMonadInternalise (ND m) [] where
  internaliseMonad :: forall u v . ND m u v -> ND m [u] v
...
mkAlignedND gen check = mkND gen (\y -> if check y then [y] else [])
...
check :: Monad m => ND m u v -> u -> [v] -- backward direction
...
children p = children' parents
 where
   children' [] = []
   children' ((p', child):parents) | p == p' = child : children' parents
   children' (_:parents) = children' parents
...
comap' :: (u -> [u']) -> ND Logic u' v -> ND Logic u v
...
grandparentsOfB c = do
  parent <- children `comap` parentOfB c
  parentOfB parent
...
exampleCheck =
  check (grandparentsOfB "Wednesday") "Grandmama"
